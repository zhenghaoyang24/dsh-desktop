...
- Title-bar icon follows the theme too: the black logo (`buildResources/logo.png`) in light mode, the white logo (`buildResources/logo-light.png`, generated by inverting the black one) in dark mode — swapped at runtime via `win.setIcon()` (CSS `invert` cannot touch the native title bar). `nativeTheme.on('updated')` re-applies on OS theme changes in `system` mode
- The two logo files are packaged inside the app (`buildResources/logo*.png` in the `files` list) so the runtime icon swap works in built artifacts
